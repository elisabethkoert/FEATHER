function setCalibrationPowermeterThorlabs(ee, Ithreshold)
...
% the berabr data and correct the .C field. Two subfields will emerge for
...
for ii = 1 : numel(L.ABR_SeriesID)
    ...
    if strcmp( B.Stim(1).modality,'Optical')==1 %we only proceed to calibration for optical measurements
        ...
        if ~isfield(B.C, 'GT') % that is to check that I have not already converted the calibration
            switch     erase(B.Stim(1).stimulusHardware," ") % I erase the whitespaces to facilitate matlab understanding.
                case   'LaserOxxiusMPA' % I have erased the whitespaces.
                    ...
                    [indexIcal]=B.C.Ical>=Ithreshold;% to only change the values that are higher than the ithreshold
                    C.Ical = B.C.Ical;
                    polyOuttmp =  polyval(p_TL_OX,B.C.Ical);
                ...
                case   'LaserObisTTL' % I have erased the whitespaces.
                    ...
                    [indexIcal]=B.C.Ical>=Ithreshold;% to only change the values that are higher than the ithreshold
                    C.Ical = B.C.Ical;
                    polyOuttmp =  polyval(p_TL_OB,B.C.Ical);
                    C.Ical(indexIcal) = polyOuttmp(indexIcal);
                    B = setC(B,C);
                    enablecache off
                    saveBerabr(B);
