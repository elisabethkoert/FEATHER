...
arguments
    ...
    ref_time double = 1
    low_filt double = 600
    high_filt double = 6000
    filt_ord double = 4
    prestimrectime double = 150
    poststimrectime double = 150
    use_artefact_removal logical = true
end
...
% get HEADER infors for this channel
headerInfo = parseNLXHeaderInfo(channel(idx_channel).data.Header);
ADBitVolts = headerInfo.ADBitVolts;
DspDelay = headerInfo.DspDelay;
...
for idx_channel= 1: size(rawFiles, 1) % parfor
    ...
    thresh_cross = (i_thresh.*datafilt(1:end-1) < i_thresh*waveform_threshold_low) & ...
                   (i_thresh.*datafilt(2:end) > i_thresh*waveform_threshold_low);
    valid_range = (1:length(thresh_cross)) > samples_pre & (1:length(thresh_cross)) < numel(datafilt)-(samples_post+1);
    wave_idcs = find(thresh_cross & valid_range');
...
%     channel(idx_channel).datafilt=datafilt; % usually commented out as this would lead to large IC objects
    ...
    %% delete data traces to keep the memory light
    channel(idx_channel).data=[];
    channel(idx_channel).data_all=[];
...
for k=1:size(rawFiles, 1)
    ...
    my_field = "elec" + string(analyzed_elec);
...
save_name = string(IC.ExpID) + "_" + string(IC.SeriesID) + "_ICME_Resort.txt";
