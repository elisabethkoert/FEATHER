function [fig,PSTH_norm,PSTH,bin_centers,resp_idx,t_onset,t_offset,peak_response_time,used_icmes] = calculatePSTHanex_oneRecordingPerAnimal(ee_list,Exp_type,stim_criteria_array,plot_bool,artefact_removal,stimdur,t_start,t_stop,PSTH_binsize,normalize_plot )
    %anex/calculatePSTHanex_oneRecordingPerAnimal calcualtes the normalized PeriStimulusTimeHistogram and calculates
    ...
    for ee_ix=1:length(ee_list)
        ...
        if isfile(fullfile( expProcDataDir(ee.ExperimenterID, ee.ExpID),'ICME',strcat("ICUserInput_", ee.ExpID, ".mat")))
            ...
            for ii = 1: numel(L.IC_SeriesID)
                 if used_this_animal==1 % onyl get 1 icme per animal
                     continue
                 end
